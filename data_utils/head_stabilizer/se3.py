...
def matrix_multiply(left, right):
    """Small matrix product without dispatching to cuBLAS."""
    return (left.unsqueeze(-1) * right.unsqueeze(-3)).sum(dim=-2)


def matrix_vector(matrix, vector):
    """Small matrix-vector product without dispatching to cuBLAS."""
...
def so3_exp(omega):
    ...
    return identity + a[..., None] * omega_hat + b[..., None] * matrix_multiply(
        omega_hat, omega_hat)
...
def se3_exp(twist):
    ...
    jacobian = identity + b[..., None] * omega_hat + c[..., None] * matrix_multiply(
        omega_hat, omega_hat)
    translation = matrix_vector(jacobian, velocity)
...
def se3_log(rotation, translation):
    ...
    jacobian_inv = identity - 0.5 * omega_hat + coefficient[..., None] * matrix_multiply(
        omega_hat, omega_hat)
...
def relative_twist(rotation, translation):
    ...
    relative_rotation = matrix_multiply(rotation[1:], previous_inv)
    relative_translation = translation[1:] - matrix_vector(
        relative_rotation, translation[:-1])
